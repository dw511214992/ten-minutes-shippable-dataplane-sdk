...
export async function getLanguage(): Promise<string[]> {
    const validLanguage = ['js', 'python', 'java'];
    while (true) {
        const languages = await getInputFromCommand(`Which language of sdk do you want to get? If you want to get multi-language, please use semicolon to separate them. (Supported Language: js, python, java): `);
        if (languages.trim() !== '') {
